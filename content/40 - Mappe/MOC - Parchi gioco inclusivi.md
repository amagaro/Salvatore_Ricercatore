...
```mermaid
graph TD
    A["MOC: Parchi gioco inclusivi"] --> B("Universal Design")
    A --> C("Spazio Sensoriale (Neurodivergenze)")
    A --> D("Impatto Sociale e Pedagogico")
    
    B --> E("Design For All")
    B --> F("Superamento Barriere (Oltre le Rampe)")
    
    C --> G("Sensory Overload")
    C --> H("Spazi Rifugio")
    
    D --> I("Outdoor Education")
    D --> J("Diritto al Rischio (Rischio Calcolato)")
```
