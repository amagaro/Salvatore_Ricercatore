...
```mermaid
graph TD
    A["Corpus Letteratura Parchi Inclusivi"] --> B("Dati Qualitativi (Maggioranza)")
    A --> C("Dati Quantitativi / KPI (Minoranza)")
    
    B --> D("Studi di Caso Osservazionali")
    B --> E("Teoria Pedagogica (PEO, UDL)")
    B --> F("Focus Group e Interviste")
    
    C --> G("Metriche Territoriali (Censimenti)")
    C --> H("Metriche di Gioco (Statistiche Tipologiche)")
```
